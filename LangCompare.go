...
func main() {
	...
	twoLangs := *getTwoLangZips(nMap) //6303
	...
	//write table of shared outliers to DB
	T1 := writeToDBT1(cMap, nMap, outliersCS, outliersSTD, outliers) //1114
	checkAdded(T1)

	//write table of chisq outliers to DB
	T2 := writeToDBT2(cMap, nMap, outliersCS, outliers) //1114
	checkAdded(T2)

	//write table of STD outliers to DB
	T3 := writeToDBT3(cMap, nMap, outliersSTD, outliers) //1114
	checkAdded(T3)
...
func writeToDBT1(c map[string][]string, n map[string][]string, oCS map[string]float64, oSTD map[string]float64, outliers []string) bool {
	pg_string := "insert pg string"
	db, err := sql.Open("postgres", pg_string)
	if err != nil {
		log.Fatal(err)
		return false
	...
	//query DB
	query, err := db.Query(`CREATE TABLE zip_code_shared_outliers (zip_code VARCHAR PRIMARY KEY, total_requests VARCHAR, request_langs TEXT, total_eng_requests VARCHAR, ` +
...
func writeToDBT2(c map[string][]string, n map[string][]string, oCS map[string]float64, outliers []string) bool {
	pg_string := "insert pg string"
	db, err := sql.Open("postgres", pg_string)
	if err != nil {
		log.Fatal(err)
	...
	//query DB
	query, err := db.Query(`CREATE TABLE zip_code_chisq_outliers (zip_code VARCHAR PRIMARY KEY, total_requests VARCHAR, request_langs TEXT, total_eng_requests VARCHAR, ` +
		`total_lang2_requests VARCHAR, census_english_perc VARCHAR, census_spanish_perc VARCHAR, census_indoeuropean_perc VARCHAR, census_asianpacific_perc VARCHAR, ` +
		`census_other_perc VARCHAR, chi_squared VARCHAR);`)
	if err != nil {
		log.Fatal(err)
		return false
	} else {
	...
	statement := `INSERT INTO zip_code_chisq_outliers (zip_code, total_requests, request_langs, total_eng_requests, total_lang2_requests, census_english_perc, census_spanish_perc, ` +
		`census_indoeuropean_perc, census_asianpacific_perc, census_other_perc, chi_squared) VALUES `
...
func writeToDBT3(c map[string][]string, n map[string][]string, oSTD map[string]float64, outliers []string) bool {
	pg_string := "insert pg string"
	db, err := sql.Open("postgres", pg_string)
	if err != nil {
		log.Fatal(err)
	...
	//query DB
	query, err := db.Query(`CREATE TABLE zip_code_stddev_outliers (zip_code VARCHAR PRIMARY KEY, total_requests VARCHAR, request_langs TEXT, total_eng_requests VARCHAR, ` +
		`total_lang2_requests VARCHAR, census_english_perc VARCHAR, census_spanish_perc VARCHAR, census_indoeuropean_perc VARCHAR, census_asianpacific_perc VARCHAR, ` +
		`census_other_perc VARCHAR, sum_std_dev VARCHAR);`)
	if err != nil {
		log.Fatal(err)
		return false
	} else {
	...
	statement := `INSERT INTO zip_code_stddev_outliers (zip_code, total_requests, request_langs, total_eng_requests, total_lang2_requests, census_english_perc, census_spanish_perc, ` +
		`census_indoeuropean_perc, census_asianpacific_perc, census_other_perc, sum_std_dev) VALUES `
...
func displayFMap(m map[string]float64) {
...
func displaySMap(m map[string][]string) {
